...
extern _openmv_data_st opmv;//任务指示的状态量
static u32 ztt_cnt = 0;//用作计数延时的量。
void INT_1ms_Task()
{	
...
static void Loop_Task_8(u32 dT_us)	//20ms执行一次，1s对应50
{
	u8 dT_ms = 20;//(u8)(dT_us *1e-3f);
	//==========================
	//
	//当发现按下按键时，等待5s，一键起飞，飞行15s后，一键降落。
	//如何解决：程序会不停执行这个过程的问题。
	//答，控制ztt_cnt的清零，每当检测到opmv.mode_sta != 0时，将ztt_cnt = 0;
	//这里先不考虑。
	...
	if ((opmv.mode_sta == 3) && (ztt_cnt>250 && ztt_cnt<=1000))
	{
	
		ztt_cnt++;
		one_key_take_off();
	}
	else if((opmv.mode_sta == 3) && (ztt_cnt<=250))
	{
		ztt_cnt++;
	}
	else if((opmv.mode_sta == 3) && (ztt_cnt>1000 && ztt_cnt<1200))
	{
		one_key_land();
		ztt_cnt = 1200;
	}
