...
static void Loop_Task_8(u32 dT_us)	//20ms执行一次
{
	...
	if((ztt > 700)&&(ztt < 950)&&(wcc == 1))
	{
	CH_N[CH_PIT] = 200;
	}
	if((opmv.mode_sta == 3)&&(lyz < 200))   //如果识别到色块
	{
	lyz ++;
	CH_N[CH_PIT] = 0;
	ztt = 950;
  ...
  if((lyz == 200)&&(ttz < 200))
	...
	if ((ttz >= 300)&&(wcc == 1)&&(rosta == 0))
	{
		Program_Ctrl_User_Set_HXYcmps(30,0);		
		Program_Ctrl_User_Set_YAWdps(70);
		if(hxm < 180000/70)
		{
			hxm += 20;
		}
