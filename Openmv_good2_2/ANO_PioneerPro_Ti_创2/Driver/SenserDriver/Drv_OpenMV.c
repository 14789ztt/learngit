...
{	
	...
	{
		if(bytedata == 0x29)//(bytedata==0x29)未确定
		{
			rec_sta++;
		}	
		else
	...
	{
		if(bytedata==0x41 || bytedata==0x42 || bytedata == 0x40  ||bytedata == 0x43 || bytedata == 0x44)
		{
			rec_sta++;
		}
		else
...
static void OpenMV_Data_Analysis(u8 *buf_data,u8 len)
{
	...
	else if(*(buf_data+3) == 0x40)
	{
		opmv.mode_sta =3;
	}
	else if (*(buf_data+3) == 0x43)
	{
		opmv.mode_sta = 4;//一键起飞
		
	}
	else if(*(buf_data+3) == 0x44)
	{
		opmv.mode_sta = 5;//什么都不做
	}
	//
	OpenMV_Check_Reset();
}
