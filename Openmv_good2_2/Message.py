...
def takeoff():
    take_off_data = bytearray([0xAA,0x29,0x05,0x43,0x01,0x00,0x00])
    take_off_data[6] =284#这个值是随着bytearray的变化而变化的。
    uart.write(take_off_data)


def Waiting():
    wait_data = bytearray([0xAA,0x29,0x05,0x40,0x01,0x00,0x00])
    wait_data[6] =281#这个值是随着bytearray的变化而变化的。
    uart.write(wait_data)


def Nothing():
    wait_data = bytearray([0xAA,0x29,0x05,0x44,0x01,0x00,0x00])
    wait_data[6] =285#这个值是随着bytearray的变化而变化的。
    uart.write(wait_data)
